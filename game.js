// Payment check logic using Razorpay
function showPaymentOverlay() {
  document.getElementById('payment-overlay').style.display = 'flex';
  document.getElementById('game-container').style.filter = 'blur(12px)';
  document.querySelector('.title-row').style.filter = 'blur(12px)';
  document.querySelector('.tabs').style.filter = 'blur(12px)';
}
function hidePaymentOverlay() {
  document.getElementById('payment-overlay').style.display = 'none';
  document.getElementById('game-container').style.filter = '';
  document.querySelector('.title-row').style.filter = '';
  document.querySelector('.tabs').style.filter = '';
}
// Only unlock if payment succeeds
if (!localStorage.getItem('paid_once')) {
  showPaymentOverlay();
  document.getElementById('rzp-button').onclick = function() {
    var options = {
      "key": "rzp_test_ff7rjHwCrEiGw0", // Replace with your Razorpay key ID
      ...
      "handler": function (response){
        // Payment successful
        localStorage.setItem('paid_once', 'true');
        hidePaymentOverlay();
        alert('Payment successful! Enjoy lifetime access.');
      },
      "prefill": {
        "name": "",
        "email": ""
